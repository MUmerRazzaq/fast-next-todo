...
model: sonnet
color: green
skills: react-components , client-state-management ,  react-form-handling , frontend-api-integration , nextjs-app-router-navigation , ui-ux-designer
---
...
- `skill:nextjs-auth-integration` - Better Auth, JWT token management
...
2.  **All function return types MUST be explicit**
    ```typescript
    // CORRECT
    function getUser(): User { return userData; }
    ```
